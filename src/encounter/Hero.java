...
public class Hero extends EncounterActor {
	...
	public Hero(String r, String c, Socket actorSocket) throws IOException{
		iNextLvlXP = 300;
		iLevel = 1;
		iXP = 0;
		iProficiencyBonus = 2;
		...
		case "lightfootHalfling":
			...
			iSize = 1;
			iMoveSpeed = 25;
			dictFeats.put("Lucky", true);dictFeats.put("Brave", true);dictFeats.put("Halfling Nimbleness", true);
			dictFeats.put("Naturally Stealthy", true);
			break;
		case "stoutHalfling": 
			iDex += 2; iCon++;
			iSize = 1;
			iMoveSpeed = 25;
			dictFeats.put("Lucky", true);dictFeats.put("Brave", true);dictFeats.put("Halfling Nimbleness", true);
			dictFeats.put("Stout Resilience", true);
			break;
		...
		case "forestGnome": 
			...
			iSize = 1;
			iMoveSpeed = 25;
			dictFeats.put("Darkvision", true);dictFeats.put("Gnome Cunning", true);
			dictFeats.put("Speak with Small Beasts", true);
			break;
		case "rockGnome": 
			iInt+=2;
			iSize = 1;
			iMoveSpeed = 25;
			dictFeats.put("Darkvision", true);dictFeats.put("Gnome Cunning", true);
			dictFeats.put("Artificer's Lore", true);dictFeats.put("Tinker", true);
			listProficiencies.add("Tinker's tools");
			break;
		...
		case "halfElf":
			...
			tempArrayList.add("Strength");tempArrayList.add("Constitution");tempArrayList.add("Dexterity");tempArrayList.add("Wisdom");tempArrayList.add("Intelligence");
			try {
				tempArrayList = prompt(pw, br, "Choose 2 ability scores to increase", tempArrayList, 2);
				...
				//TODO Add one to each chosen stat
			} catch (IOException e) {
				e.printStackTrace();
			}
			iSize = 2;
	...
	public void levelUp(Socket actorSocket) throws IOException{
		...
		if (iLevel % 4 == 0){
			...
			if(prompt(pw, br, "Choose A Feat Or Stat Increase", tempArrayList, 1).equals("New Feature")){
				...
				dictFeats.put(prompt(pw, br, "Choose A Feat Or Stat Increase", tempArrayList, 1).get(0), true);
			}
			else{
				tempArrayList = new ArrayList<String>();
				...
				ArrayList<String> tempStringArray = prompt(pw, br, "Choose 1 Stat to increase by 2, or 2 stats to increase by 1 each", tempArrayList, 2);
	...
	public ArrayList<String> prompt(PrintWriter pw, BufferedReader br, String sTitle, ArrayList<String> options, int numSelectable) throws IOException{
		...
		while(inputString.contains(",")){
			returnList.add(inputString.substring(0, inputString.indexOf(",")));
	...
	public void shortRest(Socket actorSocket){
		...
		if (iHP < iMaxHP){
			...
			try {
				...
				String diceRolledString = prompt(pw, br, "Spend Hit Dice?", tempArrayList, 1).get(0);
